...
class LineMapPublisher(Node):
    ...
    def __init__(self, line_map_file, latched=False):
        super().__init__('line_map_publisher')
        self.msg = self.read_line_map(line_map_file)
        if (latched):
            self.publisher_ = self.create_publisher(LineMap, 'line_map', QoSProfile(depth=1, durability=QoSDurabilityPolicy.TRANSIENT_LOCAL))
            self.publish_map()
        else:
...
def main(args=None):
    ...
    if (len(sys.argv) < 2):
        print("Usage: publish_line_map.py <line_map.yaml> [latched]")
        exit()
    ...
    line_map_publisher = LineMapPublisher(sys.argv[1], latched)
